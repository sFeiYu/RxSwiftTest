...
//    结构体的汇编简单很多，如果内部只是些简单处理，可以定义为结构体
...
struct 初始化 {
//    指定初始化器最完整，便捷初始化器里面强制调用指定初始化器，便捷初始化器可以省略传参
//    指定不能调用指定，便捷最终一定要调用指定，便捷可以调用便捷
//    指定必须调用父类的指定
    
//    var aa: UILabel (let aa: UILabel 也可以)
//    这种必须赋值的存储属性，必须在指定初始化器一开始就调用，且必须要赋值
//    但是便捷初始化器，必先调用指定初始化器
//    便捷初始化器是横向，指定初始化器是纵向
